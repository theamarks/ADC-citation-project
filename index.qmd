...
# separate source column into columns for each source
overlap2 <- scythe_cit %>% 
...
# combine source columns into single column that contains overlap information
# remove single source columns
overlap3 <- overlap2 %>% 
  mutate(source = apply(overlap2[3:6], 1, function(x) toString(na.omit(x)))) %>% 
  select(-scopus, -springer, -plos, -xdd)


show_col(viridis(4, option = "C"))
overlap_color <- c("#AB2494FF", "#DE6065FF", "#FCA338FF", "#F0F921FF")
ovrlp_vec <- setNames(overlap$n, as.character(overlap$source_combination))
fit <- euler(ovrlp_vec, shape = "ellipse")
plot(fit, 
     quantities = TRUE,
