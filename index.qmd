...
Pull dataset discipline semantics from Solr. 

```{r get_solr_discipline}
#| code-fold: true
...
disc_adc <- read.csv("https://raw.githubusercontent.com/NCEAS/adc-disciplines/main/adc-disciplines.csv") %>% 
...
solr_adc_sem$category <- purrr::map(solr_adc_sem$sem_annotation, function(x){
    ...
    cats <- c()
    for (i in 1:length(t)){
        z <- which(disc_adc$an_uri == t[i])
        cats[i] <- disc_adc$discipline[z]
...
disc_col_names <- colnames(disc_manual)
...
disc_adc_wide <- solr_adc_sem %>% 
  ...
  select(-sem_annotation, -datasource, -dateUploaded, -title) 
colnames(disc_adc_wide) <- disc_col_names

# join disc dataframes
#disc_all <- rows_update(disc_adc_wide, disc_manual, by = 'id' , unmatched = "ignore")

scythe_cit_disc_adc <- left_join(scythe_cit, disc_adc_wide, by = "dataset_id")
