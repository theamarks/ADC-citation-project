...
do_cit_src_2022_07_25 <- readr::read_csv(file.path("data/dataone_cits_report_2022_07_25.csv"))
...
do_cit_src_2022_07_25 <- as.data.frame(lapply(do_cit_src_2022_07_25, gsub, pattern = "*'*", replacement = ""))

do_cit_src_2022_07_25 %<>%
  rename("article_id" = source_id, # rename dataone metrics citations columns to match scythe results
         "dataset_id" = target_id) %>% 
  mutate(reporter = sub("^http.*","ORCiD", do_cit_src_2022_07_25$reporter))

do_cit_source_sum <- do_cit_src_2022_07_25 %>% 
  group_by(reporter) %>% 
  summarise(num_cit = n()) 
...
scythe_cit_new <- anti_join(unique_citations, do_cit_src_2022_07_25, by = c("article_id", "dataset_id")) %>% 
  na.omit()
...
scythe_in_dataone <- semi_join(do_cit_src_2022_07_25, unique_citations, by = c("article_id", "dataset_id"))
