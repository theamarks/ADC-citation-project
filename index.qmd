...
*SOLR query does not yet include date search term to align with `date` object. Useing `date` to save and read in .csv*
...
```{r read_saved_scythe_results}
#| code-fold: true

# read in saved scythe results for all sources
# reduces dependency on global environment objects
read_scythe_results <- function(){
...
::: callout-note
Running a second round of API queries using DOIs associated with 1st round API request errors is not automated here. Future script users will need to adjust the following code chunk. 
:::
